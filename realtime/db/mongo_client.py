...
from sms.twilio_client import Client as Twilio
import os
import pymongo
...
#        return self.client['parse']['testAlerts']
    ...
    def price_met(self, alert, event_price):
        alert_price = alert['details'].get('price')
        price_met = True
        if alert_price:
            price_met = float(event_price) <= float(alert_price)
        return price_met
    ...
    def getMatchingAlerts(self,alerts, tokenID, traits,EthPrice,rarity):
        ...
        baseToTrigger.extend([alert for alert in token_id_alerts if alert['details']['id'] == str(tokenID) and self.price_met(alert, EthPrice)])
        trait_alerts = [alert for alert in alerts if alert['type'] == 'traits' and self.price_met(alert, EthPrice)]
        ...
        baseToTrigger.extend([alert for alert in rarity_alerts if float(rarity) < float(alert['details']['rarity']) and self.price_met(alert, EthPrice)])
        price_alerts = [alert for alert in alerts if alert['type'] == 'price' and self.price_met(alert, EthPrice)]
        baseToTrigger.extend(price_alerts)
